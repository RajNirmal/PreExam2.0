...
public class Exam extends AppCompatActivity implements OnTouchListener, OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		while (i < noOfQuestions) {
			...
			img[i].setId(i);
 			img[i].setBackgroundResource(R.drawable.rounds);

			if (i + 1 == selected) {
				img[i].setBackgroundResource(R.drawable.fillrounds);
				img[i].setText(i + 1 + "");
		...
		question.setOnTouchListener(new OnSwipeTouchListener(
			...
			public void onSwipeRight() {
				...
				if (selected > 1) {
					selected--;
					refresh();
					Animation a = AnimationUtils.loadAnimation(Exam.this,
		...
		submit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (flag) {
					...
					AlertDialog.Builder b = new AlertDialog.Builder(Exam.this);
