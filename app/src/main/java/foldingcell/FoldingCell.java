...
public class FoldingCell extends RelativeLayout {
    ...
    // default values
    private final int DEF_ANIMATION_DURATION = 50000;
    ...
    protected void initializeFromAttributes(Context context, AttributeSet attrs) {
        TypedArray array = context.getTheme().obtainStyledAttributes(attrs, R.styleable.HorizontalListView, 0, 0);
        try {
            this.mAnimationDuration = array.getInt(R.styleable.HorizontalListView_animationDuration, DEF_ANIMATION_DURATION);
            this.mBackSideColor = array.getColor(R.styleable.HorizontalListView_backSideColor, DEF_BACK_SIDE_COLOR);
            this.mAdditionalFlipsCount = array.getInt(R.styleable.HorizontalListView_additionalFlipsCount, DEF_ADDITIONAL_FLIPS);
        } finally {
            array.recycle();
        }
    }
